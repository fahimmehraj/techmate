...
import { effectiveTaskReminderPolicy, isRecurringTaskReminder, nextTaskReminderAt } from "../../packages/application/src/tasks.ts";
...
describe("task reminder policy", () => {
  ...
  test("schedules one-time and recurring policies at their defined boundaries", () => {
    expect(nextTaskReminderAt({ policy: "one_day_before", dueAt, now })?.toISOString()).toBe("2026-09-09T17:00:00.000Z");
    expect(nextTaskReminderAt({ policy: "one_hour_before", dueAt, now })?.toISOString()).toBe("2026-09-10T16:00:00.000Z");
    const first = nextTaskReminderAt({ policy: "daily_until_done", dueAt, now });
    expect(first?.toISOString()).toBe("2026-09-09T17:00:00.000Z");
    expect(nextTaskReminderAt({ policy: "daily_until_done", dueAt, after: first!, now })?.toISOString()).toBe("2026-09-10T17:00:00.000Z");
  ...
  test("turns an already missed reminder into a catch-up notification", () => {
    ...
    expect(nextTaskReminderAt({ policy: "one_hour_before", dueAt, now: late })?.toISOString()).toBe(late.toISOString());
  });
});
